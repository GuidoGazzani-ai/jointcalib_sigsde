...
def multidimensional_OU(X0,N,T,Rho,kappas,thetas,sigmas,flag_aug,flag_mat):
    ...
    for i in range(1,int(N+1)):
        ...
        diffusion = np.matmul(C, Z)*np.sqrt(dt) #these are diffusions including the BM
        BMs[:,i]=np.matmul(C_1, Z)*np.sqrt(dt) #these are all BMs
...
def sample_sig_OU_multi_minimal_Janka(N,T,X0,sigmas,kappas,thetas,Rho,order_signature,flag_mat):
...
def sample_sig_OU_multi_minimal_all_mat_Janka(N,maturities,X0,sigmas,kappas,thetas,Rho,order_signature,flag_mat):
    ...
    full_sig= sample_sig_OU_multi_minimal_Janka(N,last_mat+1/365.25,X0,sigmas,kappas,thetas,Rho,order_signature,flag_mat) 
...
if int(config[-1])==1:  
   ...
   # os.chdir(r'/scratch.global/ag_cu/Codes_Guido/Randomness/n=2/config2')
   # Rho =np.load('Rho_d=4.npy')  
    Rho=np.array([np.array([1,-.999]),np.array([-.999,1])])
...
def sample_tilde_df_andQ0_multimaturities(N,maturities,mat_spx,mat_vix,X0,sigmas,kappas,thetas,Rho,order_signature,flag_mat,indices,new_tilde,keys_df,keys_df_vix,Cov,d,idx_Z_d,shuffled_integral_matrix,dict_words_numbers,wordz):
    ...
    # Raw sig
    sig=sample_sig_OU_multi_minimal_all_mat_Janka(N,maturities,X0,sigmas,kappas,thetas,Rho,2*order_signature+1,flag_mat)
